...
parser.add_argument('--input_image',  type=str, help='Path to image with person.')
parser.add_argument('--input_cloth', type=str, help='Path to target cloth image')
...
parser.add_argument('--openpose_model', default='pose_iter_440000.caffemodel', help='Path to OpenPose .caffemodel model was trained on COCO dataset.')
...
if __name__ == "__main__":
    ...
    dlist = os.listdir('static/result')
    for item in dlist:
        os.remove("static/result/"+item)
    
    name = str(random.randint(1000, 9999))
    cv2.imwrite("static/result/"+name+".jpg", 255*output)

    shutil.copy(args.input_cloth, "static/collections")